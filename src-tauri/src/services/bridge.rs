...
use crate::services::{bridge_ctl, payload, process};
...
async fn supervisor_loop(app: tauri::AppHandle) {
    ...
    loop {
        ...
        if manager_autostart::is_installed() && !cleaned_bridge_autostart {
...
async fn cleanup_oc_bridge_autostart_artifacts(_layout: &PayloadLayout) -> Result<(), ()> {
    // Windows: oc-bridge used HKCU Run value "OpenControlBridge".
    #[cfg(windows)]
    {
