...
pub fn artifact_health_for_binding(
    ...
    match binding.artifact_source {
        ArtifactSource::Installed => {
            installed_artifact_health_for_binding(layout, installed, binding)
        }
...
pub fn resolve_management_oc_bridge_exe(layout: &PayloadLayout) -> ApiResult<PathBuf> {
    if let Ok(workspace) = load_workspace_artifacts() {
        ensure_file_exists("oc_bridge_exe", &workspace.oc_bridge_exe)?;
...
pub fn resolve_oc_bridge_exe_for_binding(
    ...
    match binding.artifact_source {
        ArtifactSource::Installed => {
            let path =
                installed_oc_bridge_exe_for_tag(layout, binding.installed_pinned_tag.as_deref());
            ensure_file_exists("oc_bridge_exe", &path)?;
            Ok(path)
        }
        ArtifactSource::Workspace => {
            let path = load_workspace_artifacts()?.oc_bridge_exe;
            ensure_file_exists("oc_bridge_exe", &path)?;
            Ok(path)
        }
    }
...
pub fn resolve_management_loader_exe(layout: &PayloadLayout) -> ApiResult<PathBuf> {
    if let Ok(workspace) = load_workspace_artifacts() {
        ensure_file_exists("loader_exe", &workspace.loader_exe)?;
...
pub fn resolve_loader_exe_for_binding(
    ...
    match binding.artifact_source {
        ArtifactSource::Installed => {
            let path =
                installed_loader_exe_for_tag(layout, binding.installed_pinned_tag.as_deref());
            ensure_file_exists("loader_exe", &path)?;
            Ok(path)
        }
        ArtifactSource::Workspace => {
            let path = load_workspace_artifacts()?.loader_exe;
            ensure_file_exists("loader_exe", &path)?;
            Ok(path)
        }
    }
...
pub fn resolve_firmware_for_binding(
    ...
    match binding.artifact_source {
        ...
        ArtifactSource::Workspace => {
            resolve_firmware_for_target(layout, installed, binding.artifact_source, binding.target)
        }
    }
}
...
pub fn artifact_location_for_binding(
    ...
    match binding.artifact_source {
        ...
        ArtifactSource::Workspace => load_workspace_artifacts()
            .ok()
            .map(|workspace| match binding.target {
...
pub fn resolve_bitwig_extension() -> ApiResult<PathBuf> {
    let path = load_workspace_artifacts()?.bitwig_extension;
    ensure_file_exists("bitwig_extension", &path)?;
    Ok(path)
}
