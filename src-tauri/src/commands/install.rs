...
use crate::commands::distribution::plan_latest_install_internal;
...
pub async fn install_latest(
    ...
    for (i, a) in plan.assets.iter().enumerate() {
        ...
        let p = assets::ensure_asset_cached(&state.http, &state.layout, a).await?;
    ...
    let installed = install::apply_install(&state.layout, &plan, &cached).await?;
